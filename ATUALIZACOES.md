<!--
PROTOCOLO DESTE ARQUIVO (para o Claude Code seguir)

Este é o log de atualizações do Revisor VM. Tem duas seções:
  • PENDENTES: mudanças já feitas mas AINDA NÃO anunciadas aos sócios.
  • PUBLICADAS: histórico de tudo que já foi anunciado (o log permanente).

Quando o Igor pedir "gerar mensagem de update" (ou equivalente):
  1. Monte a mensagem de anúncio com os itens de PENDENTES, neste formato EXATO:

        Nova Versão Revisor VM no Ar!
  ...
  2. Mova esses MESMOS itens de PENDENTES para o topo de PUBLICADAS, sob um
     cabeçalho com a data de hoje (### AAAA-MM-DD).
  3. Esvazie a seção PENDENTES (deixe "_(nada pendente)_").
  Resultado: um item anunciado nunca reaparece numa próxima mensagem.

Quando uma mudança nova for feita no sistema: acrescente um item em PENDENTES,
escrito em LINGUAGEM DE BENEFÍCIO para usuário NÃO TÉCNICO (o que ele ganha,
não como foi feito). Formato do item: **Título curto:** uma frase de valor.
NUNCA use travessão (—) nos itens nem na mensagem (regra da casa). Use dois
pontos, vírgula ou conectivo natural.
-->

# Atualizações do Revisor VM
...
- **Você aprova o que o sistema aprende com você:** Ao terminar de revisar um documento, antes de gravar, aparece uma tela com tudo que o sistema entendeu das suas decisões. Você edita, aprova ou descarta cada aprendizado. Nada vira regra sem o seu aval.
